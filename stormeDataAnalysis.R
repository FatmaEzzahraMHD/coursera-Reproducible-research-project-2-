#reproductible research, week 4 project 
...
data<- read.csv("./repdata_data_StormData.csv.bz2") #load data
...
dataT$BGN_DATE <- strptime(dataT$BGN_DATE, "%m/%d/%Y %H:%M:%S")
 ...
 #According to NOAA, the data recording start from Jan. 1950. At that time, they 
 #recorded only one event type - tornado. They added more events gradually, and 
 #only from Jan 1996 they started recording all events type. Since our objective 
 #is comparing the effects of different weather events, we need only to include 
dataT <- subset(dataT, BGN_DATE > "1995-12-31")
    #B
names <- names(dataT)
names
 #Based on the documentation and preliminary exploration of the data we can conclude 
 #that there are 7 variables we are interested in regarding the two questions.
...
#Question 1 : Across the United States, which types of events (as indicated in the
